...
class Registry:
    ...
    def search(self, query: str, category: str | None = None, limit: int = 10) -> list[dict]:
        """Search entries by query tokens against tags, concepts, qt_apis, title."""
    ...
    def learning_path(
        ...
        Uses referenced-by count to rank importance — files referenced
        by many others are more foundational.
        ...
        for e in self.entries:
            ...
            if cat in lang_set or cat == "build-rules":
                relevant.add(e["file"])
        ...
        # Sort by score descending, then split into layers by thresholds
...
def _build_searchable(entry: dict) -> list[str]:
    ...
    for api in entry.get("qt_apis", []):
        parts.append(api.lower())
