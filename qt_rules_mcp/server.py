...
from qt_rules_mcp.registry import Registry
from qt_rules_mcp.repo import ensure_repo

mcp = FastMCP("qt-rules", instructions="Qt AI rules lookup — anti-patterns, correct patterns, RULE/BANNED markers")
...
@mcp.tool()
def search_rules(query: str, category: str | None = None, limit: int = 10) -> str:
    """Search Qt rules by keyword. Matches tags, concepts, qt_apis, title.

    Args:
        query: Search terms (e.g. "ownership threading QObject")
        category: Filter by category (cpp, qml, js, rust, build-rules)
...
def get_rule(file: str) -> str:
    ...
    Args:
        file: Path relative to repo root (e.g. "cpp/ownership.md")
    """
...
@mcp.tool()
def get_context(languages: list[str], topics: list[str] | None = None) -> str:
    """Get combined rules context for given languages and topics.

    Args:
        languages: Language categories (e.g. ["cpp", "qml"])
        topics: Optional concept filter (e.g. ["ownership", "threading"])
    """
    repo_path = _ensure_loaded()
...
@mcp.tool()
def get_learning_path(languages: list[str], phase: int | None = None) -> str:
    ...
    sections.insert(0, f"# Learning Path: {', '.join(languages)} — {total} rules in {total_phases} phases\n")
    return "\n".join(sections)
...
def list_rules(category: str | None = None) -> str:
    ...
    Args:
        category: Filter by category (cpp, qml, js, rust, build-rules). Omit for all.
    """
    _ensure_loaded()
